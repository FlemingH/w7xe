...
private:
    ...
    float* d_opt_theta_;    // optimal θ per beam [MAX_BEAMS]
    float* d_opt_phi_;      // optimal φ per beam [MAX_BEAMS]
    float* d_opt_rho_;      // optimal ρ_dep per beam [MAX_BEAMS]
    float* d_rho_targets_;   // target ρ per beam [MAX_BEAMS]

    hipStream_t stream_;

    // Launcher geometry (upper-port)
