# ROCM-RTPC: 实时等离子体计算平台

## ROCm Real-Time Plasma Computation Platform
...
P-EFIT 是全球唯一实际部署的 GPU 平衡重建代码（Huang et al., Nuclear Fusion 60, 2020）。以下将 ROCM-RTPC 的 kernel 实测数据与 P-EFIT 论文公开数据逐项对比：

| Kernel 类别 | P-EFIT (TITAN X) | ROCM-RTPC (RX 9070) | 对比 | 说明 |
|------------|-----------------|--------------------|----|------|
| **G-S 5 步求解器** (Step 1-5) | ~0.027 ms (129) | ~0.050 ms (129) | P-EFIT 1.9× | P-EFIT 手动 CUDA 优化更深 |
| **pflux_ 完整** (G-S + Green) | ~0.31 ms (129) | ~0.383 ms (129) | P-EFIT 1.2× | ROCM-RTPC Green FP16 优化后差距缩小 |
| **单次迭代 fit_** | ~0.375 ms (129) | ~0.38 ms (129) | **基本持平** | 含数据传输和 profile 计算 |
| **完整重建 (~10 迭代)** | ~4-7 ms (129) | **3.77 ms** (129) | **ROCM-RTPC 更快** | 预分配缓冲区 + 异步执行 |
| **65×65 完整重建** | ~3 ms | **1.56 ms** (含 FP16 Green) | **ROCM-RTPC 1.9×** | FP16+LDS 优化效果显著 |

**关键结论**:
- G-S 求解器核心 kernel (Step 1-5) 与 P-EFIT 处于同一数量级，TITAN X 的手写 CUDA 仍有微优势
- 完整 10 迭代重建 ROCM-RTPC 更快（3.77 ms vs 4-7 ms），因为：预分配收敛缓冲区、FP16 Green 矩阵 2.4× 加速、异步内存操作
- **ROCM-RTPC 是全球首个基于 AMD GPU 手写 HIP kernel 的 EFIT 实现**
...
| **ROCM-RTPC V1.0** | 0.788 ms | FP32 dense, 4× 展开 | 本项目 |
| **ROCM-RTPC V1.1** | **0.333 ms** | **FP16 + LDS tiled + 8× 展开** | **本项目** |
...
```
ROCM-RTPC 性能优化演进 (129×129, 10 迭代)
═══════════════════════════════════════════════════════════════════
